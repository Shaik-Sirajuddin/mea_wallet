...
const ForgetPassword: React.FC = () => {
  ...
  const handleForgotPassword = () => {
    if (!email) {
      setEmailError(t("auth.forget_password.email_required"));
      return;
    } else if (!utils.validateEmail(email)) {
      setEmailError(t("auth.forget_password.valid_email"));
      return;
    }
  ...
  return (
    <>
        ...
        <View className="w-full h-full max-w-5xl mx-auto px-4 pt-8 pb-10 justify-between">
            ...
            <View className="mt-3 mb-2">
              <View className="flex-row items-center gap-2 mb-3">
                ...
                <Text className="text-base font-medium text-white">
                  {t("auth.forget_password.enter_email")} <Text className="text-pink-1200"> *</Text>
                </Text>
              </View>
              <TextInput
                value={email}
                ...
                placeholder={t("auth.forget_password.enter_email")}
          ...
          <View className="items-center mt-6">
            <PrimaryButton
              ...
              text={t("auth.forget_password.confirm")}
              disabled={emailError !== null}
            />
            <View className="mt-4 mb-4">
              <TouchableOpacity onPress={() => router.replace("/signin")}>
                <Text className="text-[15px] text-gray-400">{t("auth.forget_password.sign_in")}</Text>
              </TouchableOpacity>
            </View>
            <View>
              <TouchableOpacity onPress={() => router.replace("/signup")}>
                <Text className="text-[15px] text-pink-1100">{t("auth.forget_password.sign_up")}</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
      ...
      <InfoAlert
        ...
        text={t("auth.forget_password.reset_sent")}
      />
    </>
  );
};
